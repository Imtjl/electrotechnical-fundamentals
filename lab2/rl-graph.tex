...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.88]
		\begin{axis}[
			...
			\node[anchor=west] at (axis cs:0,3.85) {\scriptsize $U_L(0+) = 4 \, \text{В}$};
		...
		\begin{axis}[
			...
			\addplot[color=black, only marks, mark=*, mark size=2pt] coordinates {(0, -50)};
			\node[
				anchor=west,
				font=\scriptsize
			] at (axis cs:-0.2,-48) {$-49.998\, \text{мА} = I(0+)$};
			...
			\node[anchor=west] at (axis cs:60.1,47) {\scriptsize $I(\infty) = 49.325 \, \text{мА}$};
		\end{axis}
	\end{tikzpicture}
\end{figure}
\FloatBarrier
