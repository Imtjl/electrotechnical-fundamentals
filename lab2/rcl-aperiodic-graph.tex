...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.88]
		...
		\begin{axis}[
			...
			% U_L (0+)
			\addplot[color=black, only marks, mark=*, mark size=1.7pt] coordinates {(300, 80)};
			\node[
				anchor=west,
				font=\scriptsize
			] at (axis cs:300,77) {$U_L(0+) = 4\, \text{В} $};

			% U_c (0+)
			\addplot[color=black, only marks, mark=*, mark size=1.7pt] coordinates {(300, -40)};
			\node[
				anchor=west,
				font=\scriptsize
			] at (axis cs:300,-41) {$U_c(0+) = -1,995\, \text{В} $};

			% I (0+)
			\addplot[color=black, only marks, mark=*, mark size=1.7pt] coordinates {(300, 0)};
			\node[
				anchor=east,
				font=\scriptsize
			] at (axis cs:303,-4) {$-0,031\, \text{мА} = I(0+)$};

			\draw[dashed, grey, very thin] (axis cs:0, 0) -- (axis cs:600, 0);
			\draw[dashed, grey, very thin] (axis cs:0, 80) -- (axis cs:300, 80);
		\end{axis}
	\end{tikzpicture}
\end{figure}
